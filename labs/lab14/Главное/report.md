...
![Файлы client.c и client2.c](image/1.png){ #fig:1 }

2. Клиенты передают текущее время с некоторой периодичностью (например, раз в пять секунд). Используйте функцию sleep() для приостановки работы клиента.

![Функция sleep() для приостановки работы клиента в файлах client.c и client2.c](image/2.png){ #fig:2 }

3. Сервер работает не бесконечно, а прекращает работу через некоторое время (например, 30 сек). Используйте функцию clock() для определения времени работы сервера. Что будет в случае, если сервер завершит работу, не закрыв канал?

![Цикл while, ограничивающий время работы сервера, и функция clock](image/3.png){ #fig:3 }
...
![Результат работы](image/4.png){ #fig:4 }
